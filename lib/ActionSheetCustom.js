...
import { Text, View, Dimensions, Modal, TouchableHighlight, TouchableNativeFeedback, Animated, ScrollView, Easing } from 'react-native'
...
class ActionSheet extends React.Component {
  ...
  get styles () {
    ...
    Object.keys(styles2).forEach((key) => {
      let arr = Object.assign({}, styles2[key])
      if (styles[key]) {
        arr = Object.assign({}, arr, styles[key])
      }
      obj[key] = [arr]
    })
    return obj
  ...
  _renderMessage () {
    ...
    return (
      <View style={styles.messageBox}>
        {
        React.isValidElement(message) ? message : (<Text style={styles.messageText}>{message}</Text>)
      }
      </View>
    )
  }
  ...
  _createButton (title, index) {
    const styles = this.styles
    const { buttonUnderlayColor, cancelButtonIndex, destructiveButtonIndex, tintColor, useRipple } = this.props
